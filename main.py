...
from neural_poisson import NeuralPoissonModel, EloBaselineModel
...
def main():
    ...
    model = NeuralPoissonModel(feature_names=feature_names, seed=42)
    retrain = '--retrain' in sys.argv

    loaded = False
    if NeuralPoissonModel.weights_exist(model_dir) and not retrain:
        print(f"  Loading cached model from {model_dir}...")
        model.load(model_dir)
        if sorted(model.feature_names) == feature_names:
            feature_names = model.feature_names
            cv = json.load(open(cv_path)) if os.path.exists(cv_path) \
                else {'mean_loss': None, 'mean_mae': None}
            loaded = True
            print("  ✓ Loaded cached weights (pass --retrain to force fresh training)")
        else:
            # Stale cache: the data's feature set changed since the model was
            # saved. Loading would silently mispredict — retrain instead.
            print("  ⚠ Cached features differ from current data — retraining from scratch")
